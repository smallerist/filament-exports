...
namespace SmallerId\FilamentExport;
...
use SmallerId\FilamentExport\Components\TableView;
use SmallerId\FilamentExport\Concerns\CanDisableTableColumns;
use SmallerId\FilamentExport\Concerns\CanFilterColumns;
use SmallerId\FilamentExport\Concerns\CanFormatStates;
use SmallerId\FilamentExport\Concerns\CanHaveAdditionalColumns;
use SmallerId\FilamentExport\Concerns\CanHaveExtraColumns;
use SmallerId\FilamentExport\Concerns\CanHaveExtraViewData;
use SmallerId\FilamentExport\Concerns\CanModifyWriters;
use SmallerId\FilamentExport\Concerns\CanShowHiddenColumns;
use SmallerId\FilamentExport\Concerns\CanUseSnappy;
use SmallerId\FilamentExport\Concerns\HasCsvDelimiter;
use SmallerId\FilamentExport\Concerns\HasData;
use SmallerId\FilamentExport\Concerns\HasFileName;
use SmallerId\FilamentExport\Concerns\HasFormat;
use SmallerId\FilamentExport\Concerns\HasPageOrientation;
use SmallerId\FilamentExport\Concerns\HasPaginator;
use SmallerId\FilamentExport\Concerns\HasTable;
use Carbon\Carbon;
...
class FilamentExport
{
  ...
  public function download(): StreamedResponse|BinaryFileResponse
  {
    ...
    if ($this->getFormat() === self::DOCX && $this->getModifyDocxWriter())
    {
      $filename = "{$this->getFileName()}.{$this->getFormat()}";
      $writer = $this->getModifyDocxWriter();
      return $writer($filename, $this->getViewData());
    }

    if (in_array($this->getFormat(), [self::CSV, self::XLSX], true))
    {
    ...
    return response()->streamDownload(fn() => print('sample export'), "{$this->getFileName()}.{$this->getFormat()}");
  }
  ...
  public static function getFormComponents(FilamentExportHeaderAction|FilamentExportBulkAction $action): array
  {
    ...
    {
      /** @var \SmallerId\FilamentExport\Components\TableView $component */
  ...
  public static function callDownload(FilamentExportHeaderAction|FilamentExportBulkAction $action, Collection $records, array $data)
  {
    return FilamentExport::make()
      ...
      ->modifyDocxWriter($action->getModifyDocxWriter())
      ->formatStates($action->getFormatStates())
      ->download();
  }
  ...
  public function getRows(): Collection
  {
    ...
    $data = $this->getDataWithStates($records);
